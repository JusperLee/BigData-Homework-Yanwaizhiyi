...
def getCityID(city_name):
    ...
    if "districtResult" not in post(CityURL, json=city_data).json().keys():
        city_res = post(CityURL, json=city_data).json()['noDistrictResult']
    if "districtResult" in post(CityURL, json=city_data).json().keys():
